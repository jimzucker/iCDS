...
<style>
.journal-icon { float: left; max-width: 225px; height: auto; margin: 0 1.5em 1em 0; }
@media (max-width: 600px) {
  .journal-icon { float: none; display: block; margin: 0 auto 1em; max-width: 60%; }
}
</style>

<img class="journal-icon" src="images/512_iCDS_Icon.png" width="225" height="225">
...
* 1 hour to submit an application to the iphone store, it is hard to get the icon and screenshots in acceptable sizes so they pass validations.
* 3 hours to sort out merging the applications, hit my first real iPhone programming specific issues, it turn out to be a fundamental concepts problem. As the sample code I used for each component was a stand alone app, the developers did things at an application object level so to merge them I had to change each testbed into a view.
...
* 2.5 hours lost, the app runs on the simulator but fails on the device ;(
* 2 hour working with the date routines in the ISDA libraries, looks like the way to go. Hit a snag the libor website is not available, time to implement some error handling and user alerts.
* 3.5 hours implemented error handling for accessing markit site, played with GUI elements and skimmed through the UI guidelines docs. - The Segment control has a couple of styles, the bar style is nice but small and hard to read.
	> Hint
...
* 7.5 hours and I am having a great new year, I have a $1.00 error, otherwise I match on the 10 odd tests I just ran through. Oddly I found some of the defaults it the sample LINUX code where wrong, not sure if I did it or it came that way. They key to sorting this was going through the xls example an running through the test matrix in the documentation for about 12 odd trades.
	> Trivia
	> I learned that MAC Office 2008 does not support VB, so I can't run the Markit plugin, need to download open office and try and get it running so I can > compare to the reference implementation.
* 2 hours working on the interface to clean it up. I found making icons for the tabbar controller particularly difficult, I learned they should be 30x30 and are complex to create, fortunately I found somebody else has solved the problem for me.
	> Hint
	> To create tabbar icons, check out "iPhone icon generator" by Scott Penberthy, it is a great tool. <<http://scottpenberthy.com/tab/>

	> Hint
	> There are a lot of icons available for free on the internet for the in phone:
...
* 2 hours discussing the project with my neighbor and getting advice on the interface design.
...
* 2 hours - Solved a problem where the entire done button was not active.
	> int
